	...
	ftnlen file_len, ftnlen line_len)
{
    ...
    extern /* Subroutine */ int chkin_(char *, ftnlen), errch_(char *, char *,
	     ftnlen, ftnlen);
    static integer index, units[RDTEXT_UNITS_SIZE];
...
/*               - The index of the file within the UNITS array. */
	    ...
	    ++n;
	    units[(i__1 = n - 1) < RDTEXT_UNITS_SIZE && 0 <= i__1 ? i__1 : s_rnge("units", 
		    i__1, "rdtext_", (ftnlen)639)] = unit;
	    index = n;
	}
	s_copy(lstfil, file, (ftnlen)255, file_len);
	lstunt = units[(i__1 = index - 1) < RDTEXT_UNITS_SIZE && 0 <= i__1 ? i__1 : s_rnge(
...
/*     read from the file without reopening it. */
    ...
    *eof = iostat < 0;
    if (iostat != 0) {
	cl__1.cerr = 0;
	cl__1.cunit = units[(i__1 = index - 1) < RDTEXT_UNITS_SIZE && 0 <= i__1 ? i__1 : 
		s_rnge("units", i__1, "rdtext_", (ftnlen)669)];
	cl__1.csta = 0;
	f_clos(&cl__1);
	i__1 = n;
	for (i__ = index + 1; i__ <= i__1; ++i__) {
	    units[(i__2 = i__ - 2) < RDTEXT_UNITS_SIZE && 0 <= i__2 ? i__2 : s_rnge("units", 
		    i__2, "rdtext_", (ftnlen)672)] = units[(i__3 = i__ - 1) < 
		    RDTEXT_UNITS_SIZE && 0 <= i__3 ? i__3 : s_rnge("units", i__3, "rdtext_", 
		    (ftnlen)672)];
	}
	--n;
...
/*        if the syntax is correct?  But just in case... */
	...
	cl__1.csta = 0;
	f_clos(&cl__1);
	if (units[(i__1 = index - 1) < RDTEXT_UNITS_SIZE && 0 <= i__1 ? i__1 : s_rnge("units"
